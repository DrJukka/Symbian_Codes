...
class CDisabledItem;
	...
	public:	
		...
		void ReFreshPlugInsL(void);
		void CheckSupArrayL(const TDesC& aName,const TUint32& aUid,RPointerArray<CSupportTypeItem>& aSupArray,CMyDBClass& aDb);
	public:
	...
	public:
		...
		void GetVersionL(const TDesC8& aData,TDes& aVersion);
	...
	private:
		...
		void GetFileHandlers(RPointerArray<CDisabledItem>& aDisabledArray);
		void GetRecognizers(RPointerArray<CDisabledItem>& aDisabledArray);
		void GetSupportedFilesArrayL();
		void CheckRecognizersL(RImplInfoPtrArray& aArray,RPointerArray<CDisabledItem>& aDisabledArray);
	private:	
		...
		TFileName								iConFile;
		RPointerArray<CFtASSItem> 				iFTOwerWriteArray;
		TInt									iDefaultHandIndex;
	};

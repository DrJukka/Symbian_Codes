...
TInt CEikFileHandler::PerformOperationL()
{
	...
	{
		if(iSourceFullNameArray->MdcaCount() > iIndex && iIndex >= 0)
		{
...
/*				if(DoSpecialDeleteL(iCurrentSource))
				{
					retcode = KErrNone;
				}*/ /* revomed because of 5th edition capability requirements*/
			}
		}
		break;
	default:
				...
				if(dlg->ExecuteLD(R_QUERY,AskBuffer))
				{
                    ...
                    if(MyEntry.IsDir()){
                        ...
                        if(dlg->ExecuteLD(R_QUERY,*Helppp))
                        {
                             ...
                             Err = filMan->Attribs(Hjelpper, SetAttMask, ClearAttMask,TTime(0),CFileMan::ERecurse);
                             CleanupStack::PopAndDestroy(filMan);
                        }
                        CleanupStack::PopAndDestroy(Helppp));
                    }

                    Err = CCoeEnv::Static()->FsSession().SetAtt(aSourceFile,SetAttMask,ClearAttMask);
...
/*	if(SourceOpen)
	...
	CleanupStack::PopAndDestroy();//server
	*/ /*removed because of 5th edition capability requirements*/

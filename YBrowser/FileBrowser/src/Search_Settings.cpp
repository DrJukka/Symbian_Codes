			...
			{	
				...
				case 8://ESearchIncludeTyp, 
					{	
						...
						{
							StringLoader::Load(Hjelpper,R_SH_STR_TYPES);
							iSearchIncludeTyptxt.Num(iSearchIncludeTyp.Count());
							iSearchIncludeTyptxt.Append(Hjelpper);
						}
					}
					break;
				case 9://ESearchAttr//
...
----------------------------------------------------------------------------
*/
void CSearchSettings::SelectionWithWildL(const TDesC& aWild)
{
	if(iSearchContainer)
	{
		iSearchContainer->SelectionWithWildL(aWild);
	}
}
/*
-----------------------------------------------------------------------------
...
void CSearchSettings::ConstructL(const TDesC& aPath,CSearchValues* aValues)
{
	...
	if(sp)
	{		
		HBufC* Buff= StringLoader::LoadLC(R_MNU_SEARCHFILES);
...
-----------------------------------------------------------------------------
*/
TBool CSearchSettings::IsFileOnReadOnlyDrive(void)
{
	TBool ret(ETrue);
	...
	if(iSearchContainer)
	{
		TFileName tmpName;
		TInt Dummy;
		iSearchContainer->GetCurrSelFileL(tmpName,Dummy);
	
		if(tmpName.Length() > 0)
		{
			TInt CurrentDrive;
			TVolumeInfo volumeInfo; 
			TChar DriveChar = tmpName[0];
...
-----------------------------------------------------------------------------
*/
TBool CSearchSettings::SearchOn(void)
{
	TBool ret(EFalse);
	
	if(iSearchContainer)
	{
		ret = iSearchContainer->SearchOn();
	}

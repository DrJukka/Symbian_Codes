...
CIconHandler::~CIconHandler()
{
	...
	iFTOwerWriteArray.ResetAndDestroy();
...
void CIconHandler::ConstructL(void)
{
	...
	TRAP(error,ReFreshFTOverWritesL());
	TRAP(error,ReFreshPlugInsL());
}
...
-----------------------------------------------------------------------------
*/	
void CIconHandler::ReFreshPlugInsL(void)
{
	RPointerArray<CDisabledItem> DisabledArray;
		
	CMyDBClass* MyDBClass = new(ELeave)CMyDBClass();
	CleanupStack::PushL(MyDBClass);
	MyDBClass->ConstructL();
	MyDBClass->GetDisabledL(DisabledArray);
	CleanupStack::PopAndDestroy(MyDBClass);
	
	// add all recognizers here
	GetRecognizers(DisabledArray);
	
	// Map all supported files to the icons
	GetSupportedFilesArrayL();
	
	// get all file handlers
	GetFileHandlers(DisabledArray);
...
-----------------------------------------------------------------------------
*/	
void CIconHandler::ReFreshFTOverWritesL(void)
{
	iFTOwerWriteArray.ResetAndDestroy();
	...
	dataBase->ReadHandlersItemsL(iFTOwerWriteArray);
	...
	{
		if(iExtraFileRecognizer[0])
		{
			CGulIcon* iccon= iExtraFileRecognizer[0]->GetIconL(0,TSize(0,0));
			...
			{
				CYBOWRRecognizer* owrRec = STATIC_CAST(CYBOWRRecognizer*,iExtraFileRecognizer[0]);			
				owrRec->ReFreshFTOverWritesL();
			}
		}
	}
}
...
-------------------------------------------------------------------------------
*/
void CIconHandler::CheckSupArrayL(const TDesC& aName,const TUint32& aUid,RPointerArray<CSupportTypeItem>& aSupArray,CMyDBClass& aDb)
{
	CDesCArrayFlat* arrtmp= new(ELeave)CDesCArrayFlat(10);
	CleanupStack::PushL(arrtmp);

	aDb.GetHandlerTypesOWRL(aName,aUid,*arrtmp);
	...
	if(arrtmp->MdcaCount())
	{
		if(aSupArray.Count())
		{
			for(TInt rr = (aSupArray.Count() - 1); rr > -1; rr--)
			{
				if(aSupArray[rr])
				{
					if(aSupArray[rr]->iIdString)
					{
						OkMai = EFalse;
						
						for(TInt ddd=(arrtmp->MdcaCount() - 1); ddd > -1; ddd--)
						{
							if(aSupArray[rr]->iIdString->Des() == arrtmp->MdcaPoint(ddd))
							{
								OkMai = ETrue;
								arrtmp->Delete(ddd);
							}
		...
		for(TInt dd=0; dd < arrtmp->MdcaCount(); dd++)
		{			
			CSupportTypeItem* SpIttem = new(ELeave)CSupportTypeItem();
			CleanupStack::PushL(SpIttem);

			SpIttem->iIdString = arrtmp->MdcaPoint(dd).AllocL();
	...
	CleanupStack::PopAndDestroy(arrtmp);
}
/*
-----------------------------------------------------------------------
-----------------------------------------------------------------------
	...
	if(aFile.Size(FilSize)  == KErrNone)
	{
		...
		if(!Added && iDocumentHandler)
		{
			...
			if(Ret != KErrNone)
			{
				...
				if(iDefaultHandIndex >= 0 && iDefaultHandIndex < iFHandlerArray.Count())
				{
					OpenWithHandlerL(*iFHandlerArray[iDefaultHandIndex],aFile,FileType);
				}
			}
		}
	}
...
{	
	...
	&& BaflUtils::FileExists(CCoeEnv::Static()->FsSession(),aFileName))
	{
		...
		if(Rec->iIdString)
		{
			for(TInt a=0; a < iFTOwerWriteArray.Count(); a++)
			{
				if(iFTOwerWriteArray[a])
				{
					if(iFTOwerWriteArray[a]->iTypeName && iFTOwerWriteArray[a]->iHandler)
					{
						TmpFileType.CopyLC(iFTOwerWriteArray[a]->iTypeName->Des());
						TmpFileType.TrimAll();
						
						if(TmpFileType.Length() && TmpFileType == Rec->iIdString->Des()
						&& BaflUtils::Parse(iFTOwerWriteArray[a]->iHandler->Des()) == KErrNone)
						{
							TRAP(Ret,
							...
							TParsePtrC Hjelpper(iFTOwerWriteArray[a]->iHandler->Des());
		...
		if(!Added && iDocumentHandler)
		{
			...
			if(Ret != KErrNone)
			{
				...
				if(iDefaultHandIndex >= 0 && iDefaultHandIndex < iFHandlerArray.Count())
				{
					OpenWithHandlerL(*iFHandlerArray[iDefaultHandIndex],aFileName,FileType);
				}
			}	
		}
	}
...
TBool CIconHandler::OpenExtraFoldersL(const TDesC& aName,TInt32& aUID)
{
	...
	for(TInt HandlerUid=0; HandlerUid < iFHandlerArray.Count(); HandlerUid++)
	{
		if(iFHandlerArray[HandlerUid])
		{
			...
			&& iFHandlerArray[HandlerUid]->iEnabled)
			{
...
-------------------------------------------------------------------------------
*/
void CIconHandler::GetRecognizers(RPointerArray<CDisabledItem>& aDisabledArray)
{	
	iExtraFileRecognizer.ResetAndDestroy();
	
	TInt Errno1(KErrNone);
	
	TRAP(Errno1,
	CYBOWRRecognizer* owrRec = new(ELeave)CYBOWRRecognizer();
	owrRec->ConstructL(); 
	iExtraFileRecognizer.Append(owrRec);
	);
	...
	CheckRecognizersL(infoArray2,aDisabledArray);
	);
...
-------------------------------------------------------------------------------
*/
void CIconHandler::CheckRecognizersL(RImplInfoPtrArray& aArray,RPointerArray<CDisabledItem>& aDisabledArray)
{
	TAny *impl;  // pointer to interface implementation
	...
	for ( TInt i = 0; i < aArray.Count(); i++ )
	{
		...
		TRAP(Errnum,
			...
			if(CurrName.Length() && CheckVersionL(aArray[i]->OpaqueData()))
			{
				...
				for(TInt d=0; d < aDisabledArray.Count(); d++)
				{
					if(aDisabledArray[d])
					{
						if(aDisabledArray[d]->iUid == ((TUint32)implementationUid.iUid)
						&& aDisabledArray[d]->iName
						&& aDisabledArray[d]->iRecognizer)
						{
							if(aDisabledArray[d]->iName->Des() == aArray[i]->DisplayName())
							{
								OkToAdd = EFalse;
							}
						}
...
void CIconHandler::GetFileHandlers(RPointerArray<CDisabledItem>& aDisabledArray)
{
	iDefaultHandIndex = -1;
	iFHandlerArray.ResetAndDestroy();
	
	CMyDBClass* TyppedDb = new(ELeave)CMyDBClass();
	CleanupStack::PushL(TyppedDb);
	
	TyppedDb->ConstructL();
	
	TBool hasDefaltHaqnd(EFalse);
	TFileName nammeDef;
	TUint32 UiidDef;
	if(TyppedDb->GetDefaultHandL(nammeDef,UiidDef))
	{
		hasDefaltHaqnd = ETrue;
	}
	...
	for ( TInt i = 0; i < infoArray.Count(); i++ )
	{
		...
		if(infoArray[i])
		{
			...
			for(TInt d=0; d < aDisabledArray.Count(); d++)
			{
				if(aDisabledArray[d])
				{
					if(aDisabledArray[d]->iUid == ((TUint32)infoArray[i]->ImplementationUid().iUid)
					&& aDisabledArray[d]->iName
					&& !aDisabledArray[d]->iRecognizer)
					{
						if(aDisabledArray[d]->iName->Des() == infoArray[i]->DisplayName())
						{
							OkToAdd = EFalse;
						}
					}
			...
			if(OkToAdd)
			{
				...
				&& ItemEnd != KErrNotFound)
				{
					...
					if(ItemEnd > 0)
					{
						if(CheckVersionL(infoArray[i]->OpaqueData().Mid(ItemStart,ItemEnd)))
						{
							if(CheckSupportedTypesL(infoArray[i]->OpaqueData().Mid(ItemStart,ItemEnd),*NewItem))
							{
								CheckSupArrayL(infoArray[i]->DisplayName(),((TUint32)infoArray[i]->ImplementationUid().iUid),NewItem->iSupArray,*TyppedDb);
								
								// check Database if we are disabled
								// also read for additional information on support
								...
								if(hasDefaltHaqnd && NewItem->iFlags & EYBFilehandlerSupportsOpenWith)
								{
									if(nammeDef.Length() && nammeDef == infoArray[i]->DisplayName() && (UiidDef == (TUint32)infoArray[i]->ImplementationUid().iUid))
									{
										iDefaultHandIndex = iFHandlerArray.Count();
									}
								}
	...
	infoArray.ResetAndDestroy();
	CleanupStack::PopAndDestroy(TyppedDb);
}
/*
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
*/
void CIconHandler::GetVersionL(const TDesC8& aData,TDes& aVersion)
{
	...
	{	
		...
		{
			aVersion.Copy(aData.Mid(VerStart,VerEnd));
		}
	}
}
/*
...
void CIconHandler::GetSupportedFilesArrayL()
{
	...
	for(TInt ty=(allDataTypes->Count()- 1); ty >=0 ; ty--)
	{
		...
		if(HelpFn2.Length())
		{
			...
			//check overwrites..
			for(TInt oo=0; oo < iFTOwerWriteArray.Count(); oo++)
			{
				if(iFTOwerWriteArray[oo])
				{
					if(iFTOwerWriteArray[oo]->iTypeName && iFTOwerWriteArray[oo]->iHandler)
					{
						HelpFn.CopyLC(iFTOwerWriteArray[oo]->iTypeName->Des());
						HelpFn.TrimAll();
							...
							if(ls.Connect() == KErrNone)
							{
								if(ls.GetAllApps() == KErrNone)
								{
									...
									do
									{
										...
										if(KErrNone == Errnono)
										{
											if(AppInfo.iFullName == iFTOwerWriteArray[oo]->iHandler->Des())
											{
												CIconMapping* NewMap = new(ELeave)CIconMapping();
												NewMap->iSystemApp 		= ETrue;
												NewMap->iIconUid.iUid   =((TUint)AppInfo.iUid.iUid);
												NewMap->iIndex	   		=-1;
											
												iItemIconArray.Append(NewMap);
											
												delete newItem->iIdString;
												newItem->iIdString = NULL;
												newItem->iIdString = HelpFn2.AllocL();
												newItem->iTypeId = (CFFileItem::EFJustFile + iItemIconArray.Count());											
											}
										}
										
									}while(KErrNone == Errnono && !newItem->iIdString);
									);
								}

...
-----------------------------------------------------------------------------
*/
void CFFolderReader::SetSettings(TSettingsItem& aSettings)
{
	TUint sortOrder(EAscending);
	
	if (aSettings.iOrder == 1) // type
	{	
		sortOrder = ESortByExt;
	}
	else if (aSettings.iOrder == 3) // size
	{	
		sortOrder = ESortBySize;
	}
	else if (aSettings.iOrder == 4) // time
	{	
	...
	iSettings.iEntryMask = attribs;
	iSettings.iEntrySort = sortOrder;
	iSettings.iShowHiddenSystem 	= aSettings.iShow;
	iSettings.iOptimize 			= aSettings.iOptimize;
	iSettings.iDoRecocnize 		= aSettings.iDoRecocnize;
...
void CFFolderReader::AppendToListL(const TEntry& aEntry,TInt32 aType)
{
	if(aEntry.iName.Find(_L("\t")) == KErrNotFound)
	{
		...
		NewItem->iModified	= (aEntry.iModified + MyLocale.UniversalTimeOffset());
					...
					for(TInt o=0;o < iOptimizeArray.Count(); o++)
					{
						if(iOptimizeArray[o])
						{
							if(iOptimizeArray[o]->iName)
							{
								...
								{
									TLocale MyLocale;
									if(iOptimizeArray[o]->iModified == ((*iFile_list)[iReadNow].iModified)+ MyLocale.UniversalTimeOffset())
									{
										DoneAlready = ETrue;

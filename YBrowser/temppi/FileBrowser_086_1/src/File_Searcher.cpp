					...
					if(FullName->Des().Length())
					{
						...
						if(OkForSearchConditions(FullName->Des(),Type))
						{
							if(iSearchValues.iSearchString || iSearchValues.iSearchString8)
							{
								...
								NewItem->iModified	= (*iFile_list)[iCurrFile].iModified;
		...
		if(KErrNone == CCoeEnv::Static()->FsSession().Entry(aFullName,MyEntry))
		{
			...
			if(ReadLength > 0)
			{
				...
				if(KErrNotFound != hhh)
				{
					...
					NewItem->iModified	= MyEntry.iModified;
...
TBool CFileSearcher::OkForSearchConditions(const TDesC& aFullName,TInt& aType)
{
	...
	{
		TTimeIntervalDays DaysTo = iSearchValues.iTimeBefore.DaysFrom(MyEntry.iModified);
		if(DaysTo.Int() > 0)
		{
		...
		else if(DaysTo.Int() == 0)
		{
			...
			iSearchValues.iTimeBefore.HoursFrom(MyEntry.iModified,HoursTo);
			if(HoursTo.Int() > 0)
			{
			...
			else if(HoursTo.Int() == 0)
			{
				...
				iSearchValues.iTimeBefore.MinutesFrom(MyEntry.iModified,MinutesTo);
				if(MinutesTo.Int() <= 0)
	...
	{
		TTimeIntervalDays DaysTo = MyEntry.iModified.DaysFrom(iSearchValues.iTimeAfter);
		if(DaysTo.Int() > 0)
		{
			// at least 1 day before set time, so lets continue.
		}
		...
		{
			TTimeIntervalHours 	 HoursTo(0);
			MyEntry.iModified.HoursFrom(iSearchValues.iTimeAfter,HoursTo);
			if(HoursTo.Int() > 0)
			{
				// at least 1 hour before set time, so lets continue.
			}
			...
			{
				TTimeIntervalMinutes MinutesTo(0);
				MyEntry.iModified.MinutesFrom(iSearchValues.iTimeAfter,MinutesTo);
				if(MinutesTo.Int() <= 0)
				{	// File made after or at the set date, so lets go out.
					return EFalse;
				}

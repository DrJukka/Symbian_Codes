...
{	
	...
	// not included in this self-signed version.
	iImsiDone = iCellIdDone = ETrue;
...
HBufC8* CInfoGetter::InfoBufferLC(const TDesC8& aKey,TBool aUseBase64)
{
	...
	TmpBuffer->Des().AppendNum((TUint32)CEikonEnv::Static()->EikAppUi()->Application()->AppDllUid().iUid,EHex);

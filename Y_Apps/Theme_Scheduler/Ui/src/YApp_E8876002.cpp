...
#include "YApp_E8876002.h"                     // own definitions
...
-----------------------------------------------------------------------------
*/
void CMgAppUi::GetThemeNameL(TUint32 aTypeId,TDes& aName,const TInt& aIndex)
{
	aName.Zero();
	...
	{
		TFileName Hjelpper;
		iMainContainer->GetImageName(Hjelpper,aIndex);
	
		if(EikFileUtils::Parse(Hjelpper) == KErrNone){
						
			TParsePtrC Hjepl(Hjelpper);
			aName.Copy(Hjepl.NameAndExt());
		}
	}
	else
	{
	    ...
	    {
	    	if(aTypeId == (TUint32)(skinInfoArray->At(i)->PID().Uid().iUid))
	    	{
				aName.Copy(skinInfoArray->At( i )->Name());
	    		break;
	    	}
		}

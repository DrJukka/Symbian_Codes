...
void CmyPsObserver::Start(TBool aDefine)
{
 	...
 	if(aDefine)
 	{
	 	if(iIntType)
			...
			r=RProperty::Define(iKey,RProperty::EByteArray,TSecurityPolicy(TSecurityPolicy::EAlwaysPass),TSecurityPolicy(TSecurityPolicy::EAlwaysPass));
 	}
...
TInt CmyPsObserver::SetPropertyL(TInt aValue)
{
	...
	err = iProperty.Set( iCategory,iKey,aValue);
...
void CmyPsObserver::RunL()
{
  	...
  	if(iIntType)
    {
    	...
    	iCallBack.PsValueChangedL(iCategory,iKey,valValue,err);
    }
    else
    {
    	TBuf8<255> bufValue;
	    TInt err = iProperty.Get(bufValue);
    	iCallBack.PsValueChangedL(iCategory,iKey,bufValue,err);	
    }
}
/*
-----------------------------------------------------------------------------

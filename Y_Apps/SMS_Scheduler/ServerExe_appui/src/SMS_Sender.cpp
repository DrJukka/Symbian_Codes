...
-----------------------------------------------------------------------------
*/
void CSMSSender::SendMsgL(HBufC* aMessage,HBufC* aRecipients,TBool aUnicodeEnabled,TBool aFlashSMS)
{
	...
	if(aMessage)
	{
		if(iUnicodeEnabled && (aMessage->Des().Length() > 70))
			iMessage.Copy(aMessage->Des().Left(70));
		else if(aMessage->Des().Length() > 160)
			iMessage.Copy(aMessage->Des().Left(160));
		else

...
// *** p.henkel: file names changed
#include "boxes-ref-symbianos.dat"

// *** p.henkel: added some braces, const modifier added
...
// *** p.henkel: added const modifier to word8
...
int aes::rijndaelKeySched (word8 k[4][MAXKC], int keyBits, int blockBits, 
	...
	while (t < (ROUNDS+1)*BC) { 
		...
		for(i = 0; i < 4; i++)
			tk[i][0] ^= aes::S[tk[(i+1)%4][KC-1]];   // *** p.henkel: added aes::
		...
		if (KC != 8)
			...
			for(j = 1; j < KC/2; j++)
				for(i = 0; i < 4; i++) tk[i][j] ^= tk[i][j-1];
			for(i = 0; i < 4; i++) 
                        	tk[i][KC/2] ^= aes::S[tk[i][KC/2 - 1]];  // *** p.henkel: added aes::
			for(j = KC/2 + 1; j < KC; j++)
				for(i = 0; i < 4; i++) tk[i][j] ^= tk[i][j-1];
	}
	/* copy values into round key array */
...
int aes::rijndaelEncrypt (word8 a[4][MAXBC], int keyBits, int blockBits, 
		word8 rk[MAXROUNDS+1][4][MAXBC])
{
	 ...
	 */
	for(r = 1; r < ROUNDS; r++) {
		Substitution(a, aes::S, BC);  // *** p.henkel: added aes::
		ShiftRows(a,0,BC);
		MixColumns(a,BC);
...
int aes::rijndaelEncryptRound (word8 a[4][MAXBC], int keyBits, int blockBits, 
...
int aes::rijndaelDecrypt (word8 a[4][MAXBC], int keyBits, int blockBits, 
		word8 rk[MAXROUNDS+1][4][MAXBC])
{
	int r, BC, ROUNDS;
...
int aes::rijndaelDecryptRound (word8 a[4][MAXBC], int keyBits, int blockBits, 
	word8 rk[MAXROUNDS+1][4][MAXBC], int rounds)

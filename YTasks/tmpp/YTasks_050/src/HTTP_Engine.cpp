...
//#include <S125News_20007FD8.rsg>
//#include "S125News.hrh" 
#include "HTTP_Engine.h"


// Used user agent for requests
_LIT8(KUserAgent, "YUpdate 1.0");
...
void CHTTPEngine::ConstructL()
{
	...
	if(err != KErrNone) 
	{
		iObserver.ClientEvent(KErrMsg,err);
		User::Leave(err);
	}
...
void CHTTPEngine::IapConnectionDoneL(TInt aError)
{
	if(aError == KErrNone)
	{
		...
		if(OkToGo)
		{
			...
			if(iPost)
			{
				...
				SetHeaderL(hdr, HTTP::ENoCache,KNoCache);
			...
			iRunning = ETrue;
			iObserver.ClientEvent(KConnecting,KErrNone);
		}
		else
		{	
			iObserver.ClientEvent(KFileError,aError);	
		}
	}
	...
	{
		iObserver.ClientEvent(KIAPError,aError);
	}
}
/*
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
*/
void CHTTPEngine::IssueHTTPGetL(const TDesC8& aUri,const TDesC8& aBody, const TDesC& aFileName, TBool aAskForIAP)
{	
	iDataReceiver = NULL;
	...
	delete iUrl;
	iUrl = NULL;
	iUrl = HBufC8::NewL(aUri.Length() + aBody.Length() + 10);
	iUrl->Des().Copy(aUri);
	iUrl->Des().Append(_L8("?"));
	iUrl->Des().Append(aBody);
...
void CHTTPEngine::CancelTransaction()
{
	if(iRunning)
	{
		...
		iObserver.ClientEvent(KTransactionCancelled,KErrNone);
...
void CHTTPEngine::MHFRunL(RHTTPTransaction aTransaction,const THTTPEvent& aEvent)
	...
	switch (aEvent.iStatus)
		...
		case THTTPEvent::EGotResponseHeaders:
			...
			iObserver.ClientEvent(text,iReceived,iOverAllSize);
			}
			break;
		...
		case THTTPEvent::EGotResponseBodyData:
			...
			if(iOverAllSize > 0)
				text.Format(KBodyPartReceived1,iReceived,iOverAllSize);
			else
				text.Format(KBodyPartReceived2,iReceived);
			...
			if (isLast)
			{
				iObserver.ClientEvent(KBodyReceived,KErrNone);
			}
		...
		case THTTPEvent::EResponseComplete:
			...
			iObserver.ClientEvent(KTransactionComplete,KErrNone);
			}
			break;
		case THTTPEvent::ESucceeded:
			...
			iObserver.ClientEvent(KTransactionSuccessful,KErrNone);
		...
		case THTTPEvent::EFailed:
			...
			iObserver.ClientEvent(KTransactionFailed,KErrNone);
...
TInt CHTTPEngine::MHFRunError(TInt aError,
	...
	iObserver.ClientEvent(KRunError,aError);
	return KErrNone;
	}
...
TBool CHTTPEngine::SetupConnectionL(TBool aAskForIAP)
{
    ...
    {
    	iObserver.ClientEvent(KConnectingIAP,KErrNone);
...
void CIAPConnector::SetupIAPConnectionL(TBool aAskForIAP)
{
	...
	iConnectPref.SetBearerSet(ECommDbBearerGPRS);
	//Sets the CommDb ID of the IAP to use for this connection
	iConnectPref.SetIapId(iapID);

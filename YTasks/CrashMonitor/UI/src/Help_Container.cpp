...
_LIT(KtxDisclaimer_text1			,"This plug-in is used for getting information on process exits in the device, and to get the process exit code. The actual monitoring is handled by background process, so it will be on even after the UI application is not running. The status of the background application is shown by the icon in the main selection list.");
_LIT(KtxDisclaimer_text2			,"Normal application exit is shown as Kill-0-Kill, any other process exit reason will also show an exit note. If only exits with errors are wanted to be shown, then this mode of viewing can be selected from the options menu.");
_LIT(KtxDisclaimer_text3			,"The show info menu option shows the time and basic information of the process exit. The list can be reset with Clear list option, and the background application can be shut down with Set off option.");

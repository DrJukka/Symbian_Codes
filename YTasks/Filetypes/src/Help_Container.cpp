...
_LIT(KtxDisclaimer_text1			,"This plug-in is used for showing and managing the MIME type mapping to the handling applications. The list shows the current handler applications (if any) to the all MIME types defined for the device."); 
_LIT(KtxDisclaimer_text2			,"The menu option Map to new app can be used for changing the handler application. Note that some file types handler application can not be changed, and this is shown by not changing the handler applications name on the list.");
_LIT(KtxDisclaimer_text3			,"The set to default mapping option can be used to set the default handler application back.");

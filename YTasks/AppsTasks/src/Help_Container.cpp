...
_LIT(KtxDisclaimer_text1			,"This plug-in is used for getting information on different application installed into the device, as well as managing different UI applications that are currently running.");
_LIT(KtxDisclaimer_text2			,"With the apps part, each installed application can be seen in a list that you can search with the search box provided in the bottom. Info menu option shows basic information of the application and the launch option allows launching the application.");
_LIT(KtxDisclaimer_text3			,"The task parts then shown active UI application tasks. Show info menu option shows basic information and switch to allows bringing the application to the foreground. Close task can be used to send the task an end command, and Kill task can be used to force the task to exit by killing it.");
